const listaGrados = async () => {
    try {
        ...
        data.grados.forEach((grados, index) => {
            const row = `
                <tr>
                    <td>${index + 1}</td>
                    <td>${grados.nombre}</td>
                    ...
                    <td>
                        ...
                        <a href="/gradosActualizar/${grados.id}/" class="btn btn-sm btn-info">
...
window.addEventListener("load", listaGrados);